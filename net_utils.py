...
def start_client(ip,port,input_x,model_type,partition_point,device):
    ...
    print(f"{model_type} Inference is done on edge devices - {edge_latency:.3f} ms")
    ...
    print(f"{model_type} Inference is done on cloud services - {cloud_latency:.3f} ms")
...
def get_bandwidth():
    ...
    :return: 网络带宽 MB/s
    """
    print("Acquiring network bandwidth，wait...")
    ...
    print(f'The current upload speed is：{str(upload)} MB/s')
    return upload
